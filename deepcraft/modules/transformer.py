...
def example_mask():
    mask = subsequent_mask(20)[0]  # 20x20 mask

    plt.figure(figsize=(6, 6))
    plt.imshow(mask, cmap='viridis', origin='upper')
    plt.title("Subsequent Mask", fontsize=14)
    plt.xlabel("Window")
    plt.ylabel("Masking")
    plt.xticks(np.arange(0, 20, 2))
    plt.yticks(np.arange(0, 20, 2))
    plt.colorbar(label='Mask (1=keep, 0=mask)')
    plt.grid(False)
    plt.tight_layout()
    plt.show()
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        x = (
            x.transpose(1, 2)
            .contiguous().view(n_batch, -1, self.h * self.d_k)
        )
        del query
        del key
        del value
